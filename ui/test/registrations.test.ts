...
import { translateEnglish, type TranslationKey } from "../src/i18n";
...
type Registry = {
  ...
  registerNavItem: (item: { path: string; icon: unknown }) => void;
  registerRoute: (
    path: string,
    component: unknown,
  ...
  registerTaskAction: (action: TaskAction & { icon: unknown }) => void;
...
type Host = {
  ...
  context: {
    ...
    subscribeActiveWorkspace: (
      listener: (workspaceId: string | undefined) => void,
    ) => () => void;
...
const ui = new Proxy({}, { get: (_, key) => String(key) }) as Record<
  string,
  unknown
>;
...
function modalHost(
  ...
  const translate = (key: string, options?: Record<string, unknown>) =>
    translateEnglish(key as TranslationKey, options);
  return {
    i18n: {
      locale: "en",
      t: translate,
...
describe("Bitbucket plugin registrations", () => {
  it("registers native repo, Link, and review surfaces", async () => {
    const source = await readFile(
      new URL("../bundle.js", import.meta.url),
      "utf8",
    );
    const sourceModule = await readFile(
      new URL("../src/bundle.ts", import.meta.url),
      "utf8",
    );
    const styles = await readFile(
      new URL("../plugin.css", import.meta.url),
      "utf8",
    );
    ...
    const navItems: Array<{ path: string; icon: unknown }> = [];
    ...
    const translationCatalogs: Array<Record<string, Record<string, string>>> =
      [];
    registrations[0]?.lifecycle.initialize(
      {
        registerTranslations: (catalogs) => translationCatalogs.push(catalogs),
        registerNavItem: (item) => navItems.push(item),
        registerRoute: (path, _component, options) =>
    ...
    expect(translationCatalogs).toEqual([
      expect.objectContaining({
        en: expect.objectContaining({ bitbucket: "Bitbucket" }),
        pseudo: expect.objectContaining({
          bitbucket: expect.not.stringMatching(/^Bitbucket$/),
        }),
      }),
    ]);
    expect(registrations[0]?.lifecycle.destroy).toBeTypeOf("function");
    expect(navItems).toEqual([expect.objectContaining({ path: "/bitbucket" })]);
    const pluginIcon = navItems[0]?.icon;
    expect(pluginIcon).toBeTypeOf("function");
    expect(
      JSON.stringify(
        (pluginIcon as (props: unknown) => unknown)({ className: "size-4" }),
      ),
    ).toContain("M3.648 4");
    expect(routes[0]?.options).toEqual(
      expect.objectContaining({
        topbar: expect.objectContaining({ icon: pluginIcon }),
      }),
    );
    ...
    expect(
      JSON.stringify(
        integrations[0]?.Component({ workspaceId: "workspace-1" }),
      ),
    ...
    const authenticationField = source.indexOf(
      'htmlFor: "bitbucket-auth-method"',
    );
    const settingsSeparator = source.indexOf(
      "ui.Separator",
      authenticationField,
    );
    const settingsActions = source.indexOf(
      'className: "bb-settings-actions"',
      settingsSeparator,
    );
    const checkConnection = source.indexOf(
      't("checkConnection")',
      settingsActions,
    );
    ...
    expect(
      source.slice(destructiveDisconnect - 100, destructiveDisconnect),
    ...
    expect(source).toContain('setMessage(t("disconnected"))');
    expect(source).toContain("setError(errorMessage(reason, t))");
    ...
    expect(source).not.toContain(
      "window.setInterval(() => void refresh(), 9e4)",
    );
    ...
    expect(source).not.toContain(
      "repositoryId: pullRequest.repositoryId, body",
    );
    expect(repositoryProviders).toHaveLength(1);
    expect(repositoryProviders[0]?.icon).toBe(pluginIcon);
    ...
    expect(taskActions).toEqual(
      expect.arrayContaining([expect.objectContaining({ icon: pluginIcon })]),
    );
    expect(
      taskActions.every(
        (candidate) => (candidate as { icon?: unknown }).icon === pluginIcon,
      ),
    ).toBe(true);
    expect(reviewProviders).toEqual([
      expect.objectContaining({ id: "bitbucket" }),
    ]);
    expect(reviewProviders[0]?.icon).toBe(pluginIcon);
    ...
    await reviewProviders[0]?.refreshAssociations?.(
      "workspace-1",
      controller.signal,
    );
    ...
    expect(mobilePanel.props).toMatchObject({
      connectionScope: "https://bitbucket.org",
      repositoryId: "repo-uuid",
      changeRequestNumber: 42,
    });
  ...
  it("registers only the supported native task-link form on desktop and mobile", async () => {
    ...
    for (const presentation of ["desktop", "mobile"] as const) {
      ...
      const link = taskActions.find(
        (action) => action.id === "link-pull-request",
      );
      ...
      expect(
        invocations.find(
          (invocation) => invocation.key === "pullrequests.link",
        ),
      ).toMatchObject({
      ...
      expect(
        invocations.some((invocation) => invocation.key === "pullrequests.get"),
      ).toBe(true);
    }
  });
});
